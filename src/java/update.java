...
//            String file = title.substring(5)+".pdf";
            ...
            try{  
                ...
                if(submitFlag.equals("updatePaper")){
                    ...
                    stmt.executeUpdate("update conference_papers set tittle='"+title+"',abstarct='"+abst+"' where paper_id="+cid+";");
                    ...
                    stmt.executeUpdate("update pc_members set pc_name='"+name+"',email='"+email+"';");
                    out.println("<script>alert('Successfully Updated !!!');");
                    out.println("window.location = 'http://localhost:8080/web/pc_members'</script>");        
                
                }else if(submitFlag.equals("updateReview")){
                    stmt.executeUpdate("update pc_members set descrption='"+des+"',recomendation='"+rec+"';");
                    out.println("<script>alert('Successfully Updated !!!');");
                    out.println("window.location = 'http://localhost:8080/web/reviews'</script>");        
                }
                    con.close();
            }catch(Exception e){ 
                out.println(e);
